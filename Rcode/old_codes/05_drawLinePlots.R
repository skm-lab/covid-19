...
drawLinePlots = function( egid = "3458", ttlstr = ""){
  gsym = as.character(unlist(mget(egid, org.Hs.egSYMBOL)))
  ttl.str = paste0("Gene - ", ttlstr, " | ", gsym)
  ...
  plot(x, y,
       ...
       ylim= range(c(case.vec.1, case.vec.2, case.vec.3)) +c(-0.2, 1),
       xlim=c(min(x), 28))
  rect(xleft=0,
...
# dir.create("Duke_line_Plots")
# Draw plots for Immunosuppression module genes for duke data
#pdf(paste0("Duke_line_Plots/", pathid,"_Line_plots_duke_data.pdf"), onefile= "T", width = 5, height = 4)
#for(id in cmn.gns){
#  drawLinePlots(egid = id)
#  #scan()
#}
#dev.off()
