# Temporary data to check expression of individual genes
# in Wilk 2020
# Sep 23rd 2020
# S K Mohapatra
#############################################################
...
# Load data from Wilk2020
load("Data/Wilk_agg_sce.rda")

x = assay(agg_sce)

metad = colData(agg_sce)
metax = metad[,c("Donor.full", "Donor.orig", "Sex", "Status", "Ventilated", "Admission")]
metax$id = gsub(" ","_",metax$Donor.full)
metax$orig = rownames(metax)
rownames(metax) = metax$id

if(identical(metax$orig, colnames(x))){
  colnames(x) = metax$id
}

# rearrange x by severity
sel = c("H1","H2","H3","H4","H5","H6","C1_1","C2","C5","C7","c1_2","C3","C4","C6")
x = x[sel,]

# Check HLA genes
which.hla = grep("HLA", rownames(x))

for(i in which.hla) {
  barplot(x[i,],
          main=rownames(x)[i],
          las=2,
          col=c(rep("darkgreen",6),rep("pink",4),rep("red",4)))
  #legend("topright", c("Severe", "Moderate", "Healthy"),
